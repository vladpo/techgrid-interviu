  ...
  new Strategy(
    ...
    async (email: string, password: string, done) => {
      await EitherAsync.liftEither(
        Email.maybe(email).toEither(badRequestError(`Invalid email address: ${email}`))
      )
        ...
        .chain((u) =>
          EitherAsync.liftEither(
            tryParse(ZPublicUser, u, () => new AppError('Ups Server error', 500))
          )
        )
...
const compareResponse = (password: string, user: AppUser): EitherAsync<string, AppUser> => {
  ...
  passport.authenticate(
    ...
    (err: AppError | undefined, user: PublicUser | false | null) => {
      if (user) {
        ...
        const cookie = refreshTokenCookie(generateJwtToken(user, JWT_REFRESH_EXP))
        return res.cookie(cookie.name, cookie.value, cookie.options).status(200).json(token)
      } else {
        err
...
const refreshTokens = (req: JWTRequest, res: Response, next: NextFunction) => {
  Maybe.fromPredicate<string>((t) => !!t)(req.cookies[COOKIE_REFRESH_TOKEN])
    ...
    .map((u) =>
      Tuple(refreshTokenCookie(generateJwtToken(u, JWT_REFRESH_EXP)), generateJwtToken(u, JWT_EXP))
    )
    .ifRight((t) =>
      res.cookie(t.fst().name, t.fst().value, t.fst().options).status(200).json(t.snd())
    )
    .ifLeft((err) => {
      res.status(err.status).json(err.message)
    })
